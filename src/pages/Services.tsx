...
import { Wrench, Settings, Fuel, Cpu, Zap, BatteryCharging, Shield, ArrowRight, Cable, MapPin, RotateCcw } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { services, type Service } from "@/data/services";

const iconMap: Record<string, React.ElementType> = {
  Wrench, Settings, Fuel, Cpu, Zap, BatteryCharging, Shield, Cable, MapPin, RotateCcw,
};
    ...
    <main className="pt-24 pb-24">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <h1 className="font-oswald font-bold uppercase text-3xl sm:text-5xl lg:text-6xl mb-4">Our Services</h1>
        ...
        {categories.map((cat) => (
          <section key={cat} className="mb-16">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-end gap-1 sm:gap-4 mb-6 border-b border-border pb-4">
                <h2 className="font-oswald font-bold uppercase text-2xl sm:text-3xl">{cat}</h2>
                <p className="font-barlow text-muted-foreground text-sm sm:pb-1">{categoryDesc[cat]}</p>
              </motion.div>
        ...
        <motion.div
          ...
          transition={{ duration: 0.5 }}
        >
          <h3 className="font-oswald font-bold uppercase text-xl sm:text-2xl mb-3">Don't see what you need?</h3>
          ...
          <div className="flex justify-center gap-4 flex-wrap">
            ...
            <Button asChild variant="outline" className="font-oswald uppercase tracking-wider">
              <a href="tel:+639496816756">Call Us</a>
            </Button>
          </div>
        </motion.div>
      </div>
